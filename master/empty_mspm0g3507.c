...
#if !H2026_SPEED_TUNING_BUILD && !H2026_LINE_TUNING_BUILD
/*
...
static uint32_t g_oled_last_render_ms;
static uint32_t g_oled_last_tx_ms;
static Button g_stop_button;
static uint32_t g_trial_id;
static uint32_t g_trial_started_ms;
static uint32_t g_last_button_event_count;
static uint32_t g_last_bt_command_ms;
static bool g_trial_active;

static bool H2026_StopButtonRead(void *context)
{
    (void)context;
    return TiMspm0Platform_ReadKey3Level();
}
...
static void H2026_DrawStatus(const CarFirmware *firmware)
{
    ...
    (void)TB6612_DriveGetSpeedLoopStatus(
        (const TB6612Drive *)firmware->config.drive.context, &speed);
    (void)snprintf(line, sizeof(line), "%s B2 %s",
                   H2026_TrackName(firmware),
                   H2026_RunStateName(firmware));
    (void)OLED_ShowString(0U, 0U, line);
    H2026_DrawCalibration(firmware);

    (void)snprintf(line, sizeof(line), "SEG:%u PH:%u",
                   (unsigned)firmware->app.executor.index,
                   (unsigned)firmware->app.executor.track_phase);
    (void)OLED_ShowString(0U, 2U, line);
    ...
    (void)snprintf(line, sizeof(line), "RPM:%+4ld/%+4ld",
                   ...
                   (long)H2026_ClampDisplayRpm(speed.measured_right_rpm));
    (void)OLED_ShowString(0U, 5U, line);
    if ((g_last_bt_command_ms != 0U) &&
        ((uint32_t)(firmware->last_tick_ms - g_last_bt_command_ms) < 3000U)) {
        (void)snprintf(line, sizeof(line), "BT:OK ARC:%ld",
                       (long)firmware->app.config
                           .arc_effective_track_width_mm);
    } else {
        (void)snprintf(line, sizeof(line), "ARC:%ld LIM:%ld",
                       (long)firmware->app.config
                           .arc_effective_track_width_mm,
                       (long)firmware->app.config
                           .arc_line_max_correction_mm_s);
    }
    (void)OLED_ShowString(0U, 6U, line);
    (void)snprintf(line, sizeof(line), "F:%08lX",
                   (unsigned long)faults);
...
static void H2026_RunFirmware(void)
{
    ...
    VofaTelemetry_Init();
    Button_Init(&g_stop_button, H2026_StopButtonRead, 0, true, 20U);
    NVIC_ClearPendingIRQ(UART_VOFA_INST_INT_IRQN);
    NVIC_EnableIRQ(UART_VOFA_INST_INT_IRQN);
    VofaTelemetry_SendBanner();
    ...
    while (1) {
        ...
        bool command_event;
        bool state_event;
        bool was_running = g_firmware.app.executor.running;

        Button_Update(&g_stop_button, now_ms);
        command_event = VofaTelemetry_ProcessCommands(&g_firmware, now_ms);
        if (command_event) {
            g_last_bt_command_ms = now_ms;
            g_oled_dirty = true;
        }
        CarFirmware_Tick(&g_firmware, now_ms);
        if (Button_TakePressedEvent(&g_stop_button) &&
            g_firmware.app.executor.running) {
            CarApp_Stop(&g_firmware.app, CAR_FAULT_NONE, now_ms);
            g_firmware.config.drive.stop(g_firmware.config.drive.context);
            g_firmware.drive_active = false;
            g_firmware.output.motor = (CarMotorCommand){0};
        }
        faults = g_firmware.hardware_faults | g_firmware.output.faults;
        if ((g_firmware.last_button_action == CAR_BUTTON_ACTION_ARM_OK) &&
            !g_trial_active && g_firmware.app.executor.running) {
            g_trial_id++;
            g_trial_started_ms = now_ms;
            g_trial_active = true;
            VofaTelemetry_SendLineTrialArm(g_trial_id, now_ms);
        }
        if (g_firmware.button_event_count != g_last_button_event_count) {
            g_last_button_event_count = g_firmware.button_event_count;
            if ((g_firmware.last_button_action ==
                 CAR_BUTTON_ACTION_ARM_REJECTED) ||
                (g_firmware.last_button_action ==
                 CAR_BUTTON_ACTION_GRAY_REQUIRED)) {
                VofaTelemetry_SendLineTrialReject(
                    g_firmware.last_button_action,
                    g_firmware.last_arm_status, now_ms);
            }
        }
        if (g_trial_active && was_running &&
            !g_firmware.app.executor.running) {
            VofaTelemetry_SendLineTrialDone(
                g_trial_id, now_ms - g_trial_started_ms, faults,
                g_firmware.app.executor.last_exit_reason);
