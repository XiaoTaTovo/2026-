...
| HOST | 通过 | 严格 C11 路线、终点窗口、丢线、时间回绕、安全与看门狗测试 |
| SYSCONFIG | 通过 | SysConfig 1.28.0 严格生成，0 warning / 0 error |
| BUILD | 通过 | CCS 21.0.0 + TI Arm Clang 5.1.1，0 warning / 0 error |
| PROGRAM | 未执行 | 未连接、复位或烧录开发板 |
| BOARD | 未执行 | 方向、编码器符号、急停、断线保护和传感器均待实测 |
...
PA8/PA9 的 UART1 分配只因既有 SysConfig/引脚冻结而保留；运行源码不启用、不轮询、
不解析它，也不从它读取编码器。
...
2. 保持小车静止，等待 OLED 显示 `IMU:OK`。
3. 全部红外探头放在白底，按一次 KEY3；采满 16 帧后放到黑面，再按 KEY3。
4. OLED 显示 `GC:OK` 后才允许 KEY1 启动。运行中再次按 KEY1 会急停并锁存故障。
5. KEY2 不切换模式，避免误入未实现任务。
