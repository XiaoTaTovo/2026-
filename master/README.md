...
历史 `Debug` 目录曾残留 SHA-256 为
`9F057A06E2FBBB586D9114D9EDA21620EA5016139A57C7B134C18DD1ADE55786` 的最小诊断镜像；
它只在 KEY1 后以 30% 开环前进 2 s，不能巡线。当前已执行 clean/full build，下面记录的
新 `.out` 才是正式 B2 巡线镜像。

不要按 CCS 中的同名项目判断固件。旧工作区
`C:\Users\taowz\workspace_ccstheia\empty_mspm0g3507_nortos_ticlang` 当前默认仍是
`PROJECT_MODE_GRAY_ADC_DEBUG`，其 SHA-256 为 `E01795A5504F15FF5B4A1361383FDFDB44B8A84C184AF79F583A59412264374E`
的 `.out` 只用于 ADC/传感诊断，也不能执行本题正式 B2。
...
默认 `150 mm` 偏置和 `80 mm` 搜索量对应 `1340.8~1500.8 mm`。A 横线初值要求至少
3 路连续有效、有效通道平均归一化黑度不低于 `600/1000`，并连续确认两帧；普通 1~2 路
巡线不会被当成终点。若首帧恰好位于上边界，允许下一帧完成确认。确认后继续保持 R500 曲率前进
...
- A 点判据要求连续相邻通道、总置信度、有效通道平均黑度和连续帧数同时满足，拒绝普通
  1~2 路巡线与离散噪点；VOFA 所有帧和回复都做写入边界检查，空间不足时丢弃整帧并增加
  drop 计数。
...
`7F99933889F7DFE095AD9BB204764295C63F262D0AAB576E97D14B3ACBA76AAB`，文件大小
`519884 Byte`；Flash 使用 `40096/131072 Byte`，SRAM 使用 `5286/32768 Byte`
（含 512 Byte 栈）。正式距离段必须实际达到目标里程；最终停车、完成标志和成绩时间在同一
20 ms 控制拍锁存。
