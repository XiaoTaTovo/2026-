...
{
    return (firmware != 0) && !CarApp_IsRunning(&firmware->app) &&
                                         ...
                                         uint32_t now_ms)
{
    ...
    if (strcmp(command, "DONE") == 0) {
        if (CarFirmware_CompleteExternalTask(firmware, now_ms) == CAR_OK) {
            VofaTelemetry_SendOk("DONE");
        } else {
            VofaTelemetry_SendError("EXTERNAL_TASK_NOT_RUNNING");
        }
        return true;
    }
    ...
    if (strcmp(command, "HELP") == 0) {
        VofaTelemetry_SendText(
            "#HELP STOP DONE PARAMS SPDKP SPDKI SPDKD SPDLIMIT LINEKP "
            "SEARCHMM HELP\r\n");
        return true;
    }
...
{
    VofaTelemetry_SendText(
        "#H2026_FINAL_FIREWATER_V2 fields=48 period_ms=100 "
        "baud=115200\r\n");
    VofaTelemetry_SendText(
                             ...
                             uint32_t uptime_ms)
{
    ...
    VofaTelemetry_AppendU32Field(&writer,
        CarApp_IsRunning(&firmware->app) ? 1U : 0U, &first);                  /* I2 */
    VofaTelemetry_AppendU32Field(&writer,
        CarApp_IsDone(&firmware->app) ? 1U : 0U, &first);                     /* I3 */
    VofaTelemetry_AppendU32Field(&writer,
