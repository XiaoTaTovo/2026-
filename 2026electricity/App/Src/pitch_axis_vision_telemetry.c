...
#define PITCH_VISION_STATUS_PERIOD_MS 500U
...
static bool service_boot(PitchAxisVisionTelemetry *telemetry)
{
    ...
    switch (telemetry->boot_line)
    {
        ...
        case 1U: sent = write_text(telemetry, "PITCH_VISION_MOTION=GATED_AUTO\r\n"); break;
        case 2U: sent = write_text(telemetry, "PITCH_VISION_CONTROL=BALL_PD_TO_RPM\r\n"); break;
        case 3U: sent = write_text(telemetry, "PITCH_VISION_SPEED_LIMIT_RPM=50\r\n"); break;
        case 4U: sent = write_text(telemetry, "PITCH_VISION_DEADBAND_MM=5.0\r\n"); break;
        case 5U: sent = write_text(telemetry, "PITCH_VISION_STALE_LIMIT_MS=150\r\n"); break;
        default: return true;
    }
...
static bool write_status(PitchAxisVisionTelemetry *telemetry)
{
    ...
    length = append_text(message, sizeof(message), length, ",motion=GATED_AUTO,valid=");
    ...
    length = append_u32(message, sizeof(message), length, (uint32_t)report.observation.reason);
    ...
    length = append_text(message, sizeof(message), length, ",max_age_ms=");
    length = append_u32(message, sizeof(message), length, report.maximum_observation_age_ms);
    ...
    length = append_text(message, sizeof(message), length, ",ball_v_0p1mm_s=");
    length = append_i16(message, sizeof(message), length, report.ball_velocity_0_1mm_s);
    length = append_text(message, sizeof(message), length, ",rpm_0p01=");
    length = append_i16(message, sizeof(message), length, report.control_output_0_01rpm);
    ...
    length = append_text(message, sizeof(message), length, ",speed_rpm=");
    length = append_u32(message, sizeof(message), length, report.command_speed_rpm);
