...
static void restart_after_invalid_frame(ChassisFeedforwardParser *parser)
{
         ...
         ++index)
    {
        ...
        {
            uint8_t retained_length = (uint8_t)(parser->buffer_length - index);
    ...
    if (parser->buffer[parser->buffer_length - 1U] ==
        CHASSIS_FEEDFORWARD_SOF_0)
    {
    ...
    const ChassisFeedforwardSample *sample)
{
    ...
    /* First-order small-angle candidate only. PITCH_FF remains locked until
     * the ball/beam sign and scale tests establish the real mapping. */
